...
def get_confirm_order_keyboard(order_id, language='ru'):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Подтвердить заказ" if language == 'ru' else "✅ Confirm Order", callback_data=f"confirm_{order_id}")],
        [InlineKeyboardButton("❌ Отменить" if language == 'ru' else "❌ Cancel", callback_data="cancel_order")]
    ])
